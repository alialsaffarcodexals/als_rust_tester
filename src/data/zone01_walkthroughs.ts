...
export const zone01Walkthroughs: Record<string, SolutionStep[]> = {
  ...
  rpn: [
    { code: `pub fn rpn(expr: &str) -> f64 {\n    let mut stack: Vec<f64> = Vec::new();`, explain: `Evaluate the reverse-Polish expression and return the numeric result. A stack holds the operands as f64 (so division gives exact results).` },
    { code: `    for token in expr.split_whitespace() {`, explain: `split_whitespace() reads each token and ignores any extra spaces between them.` },
    { code: `        match token {\n            "+" | "-" | "*" | "/" | "%" => {\n                let b = stack.pop().unwrap();\n                let a = stack.pop().unwrap();`, explain: `An operator consumes the top two operands. Pop b first then a — order matters for - and /: it is a OP b, not b OP a.` },
    { code: `                stack.push(match token {\n                    "+" => a + b,\n                    "-" => a - b,\n                    "*" => a * b,\n                    "/" => a / b,\n                    "%" => a % b,\n                    _ => unreachable!(),\n                });\n            }`, explain: `Apply the operator and push the result back. All five required operators are covered (including % which is easy to forget).` },
    { code: `            num => stack.push(num.parse::<f64>().unwrap()),\n        }\n    }`, explain: `Any non-operator token is a number; parse it and push it onto the stack.` },
    { code: `    stack.pop().unwrap()\n}`, explain: `Once every token is processed, the single remaining value on the stack is the answer. (A CLI wrapper reads the expression from argv and prints this.)` },
  ],
  ...
  blood_types_s: [
    ...
    { code: `#[derive(Debug, PartialEq, Eq, Hash, Clone, Copy)]\npub struct BloodType {\n    pub rh_factor: RhFactor,\n    pub antigen: Antigen,\n}`, explain: `Combine those two into the BloodType struct. Declare rh_factor before antigen so the derived Debug prints "BloodType { rh_factor: ..., antigen: ... }" as the tester expects.` },
