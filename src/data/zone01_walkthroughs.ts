...
export const zone01Walkthroughs: Record<string, SolutionStep[]> = {
  ...
  rpn: [
    { code: `use std::env;\n\nfn main() {`, explain: `Import std::env so we can read command-line arguments. main() is the entry point — the exercise is a binary, not a library function.` },
    { code: `    let args: Vec<String> = env::args().collect();\n    if args.len() != 2 {\n        println!("Error");\n        return;\n    }`, explain: `Collect all arguments into a Vec. args[0] is always the program name, so args.len() == 2 means exactly one user-supplied argument. Zero args, or two or more, all print "Error" and exit.` },
    { code: `    let expr = args[1].trim();\n    if expr.is_empty() {\n        println!("Error");\n        return;\n    }`, explain: `Grab the expression string and trim surrounding whitespace. An empty string (cargo run "") is invalid — print "Error" and stop before we even try to evaluate.` },
    { code: `    match rpn(expr) {\n        Ok(result) => println!("{}", result),\n        Err(_) => println!("Error"),\n    }\n}`, explain: `Call the rpn helper. If it returns Ok we print the number; if it returns Err we print "Error". Closing the main function here.` },
    { code: `\nfn rpn(expr: &str) -> Result<i64, ()> {\n    let mut stack: Vec<i64> = Vec::new();`, explain: `The helper returns Result<i64, ()> — Ok(number) on success, Err(()) on any invalid input. The stack holds i64 operands (the spec says operands fit in i64).` },
    { code: `    for token in expr.split_whitespace() {\n        match token {\n            "+" | "-" | "*" | "/" | "%" => {`, explain: `split_whitespace() satisfies the spec: "extra spaces must be ignored". The operator arm covers all five required operators — including % (modulo) which is easy to forget.` },
    { code: `                if stack.len() < 2 {\n                    return Err(());\n                }\n                let b = stack.pop().unwrap();\n                let a = stack.pop().unwrap();`, explain: `Before applying any operator we must have at least two operands. If not, the expression is malformed. Pop b first (top of stack), then a — order matters for subtraction and division: a - b, not b - a.` },
    { code: `                let result = match token {\n                    "+" => a + b,\n                    "-" => a - b,\n                    "*" => a * b,\n                    "/" => { if b == 0 { return Err(()); } a / b }\n                    "%" => { if b == 0 { return Err(()); } a % b }\n                    _ => unreachable!(),\n                };\n                stack.push(result);\n            }`, explain: `Compute the result and push it back. Division and modulo by zero would panic in Rust, so we catch that and return Err. The _ arm can never be reached — the outer match only lets the five operators in.` },
    { code: `            num => {\n                match num.parse::<i64>() {\n                    Ok(n) => stack.push(n),\n                    Err(_) => return Err(()),\n                }\n            }\n        }\n    }`, explain: `Any token that is not an operator must be a number. If parse::<i64>() fails (e.g. "ksd"), the expression is invalid and we return Err immediately.` },
    { code: `    if stack.len() == 1 {\n        Ok(stack.pop().unwrap())\n    } else {\n        Err(())\n    }\n}`, explain: `After processing every token, a valid RPN expression leaves exactly one value on the stack. That value is the answer. More than one value means the expression had too many operands (e.g. "1 2 3 4 +"), which is also an error.` },
  ],
